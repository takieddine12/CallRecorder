...
public class RepositoryImpl implements Repository {
    ...
    private Contact populateContact(Cursor cursor) {
        ...
        contact.setPhoneNumber(cursor.getString(cursor.getColumnIndex(ContactsContractLocal.Contacts.COLUMN_NAME_NUMBER)));
        contact.setContactName(
                cursor.getString(cursor.getColumnIndex(ContactsContractLocal.Contacts.COLUMN_NAME_CONTACT_NAME)));
        contact.setPhotoUri(
                cursor.getString(cursor.getColumnIndex(ContactsContractLocal.Contacts.COLUMN_NAME_PHOTO_URI)));
        contact.setPhoneType(
                cursor.getInt(cursor.getColumnIndex(ContactsContractLocal.Contacts.COLUMN_NAME_PHONE_TYPE)));
        contact.setId(cursor.getLong(cursor.getColumnIndex(ContactsContractLocal.Contacts._ID)));
        return contact;
    }
    ...
    public List<Contact> getAllContacts() {
        ...
        Cursor cursor = database.
                query(ContactsContractLocal.Contacts.TABLE_NAME, null, null, null, null, null, null);
...
//    }
    ...
    public Long getHiddenNumberContactId() {
        Cursor cursor = database.query(ContactsContractLocal.Contacts.TABLE_NAME, new String[]{ContactsContractLocal.Contacts._ID},
                ContactsContractLocal.Contacts.COLUMN_NAME_NUMBER + " is " + "NULL", null, null, null, null);

        if (cursor != null && cursor.moveToFirst()) {
            long id = cursor.getLong(cursor.getColumnIndex(ContactsContractLocal.Contacts._ID));
            cursor.close();
            return id;
        } else
            return null;
    ...
    private ContentValues createContactContentValues(Contact contact) {
        ...
        values.put(ContactsContractLocal.Contacts.COLUMN_NAME_NUMBER, contact.getPhoneNumber());
        values.put(ContactsContractLocal.Contacts.COLUMN_NAME_CONTACT_NAME, contact.getContactName());
        values.put(ContactsContractLocal.Contacts.COLUMN_NAME_PHOTO_URI, contact.getPhotoUri() == null ?
                null : contact.getPhotoUri().toString());
        values.put(ContactsContractLocal.Contacts.COLUMN_NAME_PHONE_TYPE, contact.getPhoneTypeCode());
        return values;
    }
    ...
    public void insertContact(Contact contact) throws SQLException {
        ...
        long rowId = database.insertOrThrow(ContactsContractLocal.Contacts.TABLE_NAME, null, values);
        contact.setId(rowId);
    }
